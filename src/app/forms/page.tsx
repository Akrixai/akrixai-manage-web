...
export default function FormsPage() {
  ...
  async function handleAddForm(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      if (newForm && !newForm.error) {
        ...
        setError(newForm?.error || "Failed to add form");
      }
    } else {
  ...
  async function handleEditSave(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      if (updated && !updated.error) {
        ...
        setError(updated?.error || "Failed to update form");
      }
    } else {
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-5xl mx-auto px-2 sm:px-6">
      ...
      <form onSubmit={handleAddForm} className="flex flex-col sm:flex-row flex-wrap gap-4 items-end bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4 shadow-sm w-full">
        ...
        <div className="flex flex-col gap-1 w-full sm:w-auto min-w-0 flex-1">
          ...
          <input className="border border-[var(--border)] rounded px-3 py-2 w-full min-w-0" value={excelLink} onChange={e => setExcelLink(e.target.value)} />
        </div>
        <div className="flex flex-col gap-1 w-full sm:w-auto min-w-0 flex-1">
          ...
          <tbody>
            {filteredForms.map(form => (
              <tr key={form.id} className="border-t border-[var(--border)]">
                <td className="px-4 py-2 break-words max-w-[120px]">{form.name}</td>
                <td className="px-4 py-2 break-words max-w-[160px]"><a href={form.link} target="_blank" rel="noopener noreferrer" className="text-[var(--primary-light)] underline">{form.link}</a></td>
                <td className="px-4 py-2 break-words max-w-[160px]">{form.excel_link ? <a href={form.excel_link} target="_blank" rel="noopener noreferrer" className="text-[var(--primary-light)] underline">{form.excel_link}</a> : ""}</td>
                <td className="px-4 py-2 break-words max-w-[120px]">{form.notes}</td>
                <td className="px-4 py-2 whitespace-nowrap">{new Date(form.created_at).toLocaleString()}</td>
