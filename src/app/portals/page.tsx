...
export default function PortalsPage() {
  ...
  async function handleEditSave(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setPortals(portals.map(p => p.id === updated.id ? updated : p));
      setEditPortal(null);
  ...
  async function handleDeleteConfirm() {
    ...
    if (res.ok) {
      setPortals(portals.filter(p => p.id !== deleteId));
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <form onSubmit={handleAddPortal} className="flex flex-col sm:flex-row gap-4 items-end bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4 shadow-sm">
        ...
        <div className="flex flex-col gap-1 w-full sm:w-auto">
          ...
          <input className="border border-[var(--border)] rounded px-3 py-2 font-mono" value={credentials} onChange={e => setCredentials(e.target.value)} placeholder='{"user":"admin","pass":"secret"}' />
        </div>
        <div className="flex flex-col gap-1 w-full sm:w-auto">
          ...
          <tbody>
            {filteredPortals.map(portal => (
              <tr key={portal.id} className="border-t border-[var(--border)]">
                ...
                <td className="px-4 py-2"><a href={portal.link} target="_blank" rel="noopener noreferrer" className="text-[var(--primary-light)] underline">{portal.link}</a></td>
                <td className="px-4 py-2"><pre className="whitespace-pre-wrap text-xs font-mono">{portal.credentials ? JSON.stringify(portal.credentials, null, 2) : ""}</pre></td>
                <td className="px-4 py-2">{portal.notes}</td>
                <td className="px-4 py-2">{new Date(portal.created_at).toLocaleString()}</td>
